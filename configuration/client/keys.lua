...
local clientKeys = awful.util.table.join(mapkey(mappings.client_swap_master, function(c)
  c:swap(awful.client.getmaster())
end, {description = 'Promote to master window', group = 'Windows'}), mapkey(mappings.client_full_screen, function(c)
...
end, {description = "Previous window in tag", group = "Windows"}), mapkey(mappings.client_select_next, function(_)
  local selection = awful.client.next(1)
  if selection then
    client.focus = selection
    selection:raise()
  end
end, {description = "Next window in tag", group = "Windows"}), mapkey(mappings.client_minimize, function(c)
  c.minimized = true
