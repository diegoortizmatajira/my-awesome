...
awful.key({modkey}, 'g', function() awful.spawn('custom-nordvpn-menu') end, {description = 'Nordvpn Menu', group = 'awesome'}),
awful.key({modkey}, 'p', function() awful.spawn('custom-layout') end, {description = 'Display Layout', group = 'awesome'}),
awful.key({modkey}, 'Escape', awful.tag.history.restore, {description = 'go back', group = 'Workspace'}),
-- Window Focus
...
-- Minimize/Maximize
awful.key({modkey}, "#117", function()
    local c = client.focus
    if c then
        c.minimized = true 
    end 
end , {description = "Minimize", group = "Windows"}),
awful.key({modkey}, "#112", function()
    local c = client.focus
    if c then
...
awful.key({modkey}, 'PageUp', function() awful.client.focus.global_bydirection('left') end, {description = 'Focus window on the left', group = 'Windows'}),
awful.key({modkey}, 'PageDown', function() awful.client.focus.global_bydirection('left') end, {description = 'Focus window on the left', group = 'Windows'}),

-- Modkey + Shift
awful.key({modkey, 'Shift'}, 'Up', awful.tag.viewprev, {description = 'View previous tag', group = 'Workspace'}),
awful.key({modkey, 'Shift'}, 'Down', awful.tag.viewnext, {description = 'View next tag', group = 'Workspace'}),
-- Default Window focus
awful.key({modkey}, 'Delete', function() awful.spawn('custom-askpoweroptions') end, {description = 'Ask Exit options', group = 'awesome'}),
...
awful.key({modkey}, 'c', function() awful.util.spawn(apps.default.editor) end, {description = 'Open a text/code editor', group = 'launcher'}),
awful.key({modkey}, 'b', function() awful.util.spawn(apps.default.browser) end, {description = 'Open a browser', group = 'launcher'}),
-- Standard program
...
end, {description = 'restore minimized', group = 'Windows'}),
-- Dropdown application
awful.key({modkey, 'Shift'}, 'Return', function() _G.toggle_quake() end, {description = 'dropdown terminal', group = 'launcher'}),
awful.key({modkey}, 'z', function() _G.toggle_quake() end, {description = 'dropdown terminal', group = 'launcher'}),
-- Brightness
