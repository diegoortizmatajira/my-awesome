...
local function mapping_for(keymap, key, use_lower)
  local k = key
  if use_lower then k = k:lower() end
  for _, mapping in ipairs(keymap) do
    local m = mapping[1]
    if use_lower then m = m:lower() end
    if m == k or (aliases[k] and m == k) then return mapping end
  end
  return nil
end
...
modalbind.default_keys = {{"Escape", modalbind.close_box, "Exit Modal"}, {"Return", modalbind.close_box, "Exit Modal"}}
...
local function merge_default_keys(keymap)
  ...
  for j, k in ipairs(modalbind.default_keys) do
    ...
    if not no_add then table.insert(result, k) end
  end
  for _, m in ipairs(keymap) do table.insert(result, m) end
  return result
end
...
function modalbind.grab(options)
  ...
  keygrabber.run(function(mod, key, event)
    ...
    if mapping then
      ...
      if mapping["stay_in_mode"] ~= nil then stay_in_mode = mapping["stay_in_mode"] end

      if stay_in_mode then
        modalbind.grab{keymap = keymap, name = name, stay_in_mode = true, args = args, use_lower = use_lower}
      else
        nesting = nesting - 1
      ...
      if verbose then print("Unmapped key: \"" .. key .. "\"") end
    end
